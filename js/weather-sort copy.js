...
function arrange_cities() {
    ...
    btn_north.addEventListener('click', function () {
        region.innerHTML = '';
        cities[1].forEach((city, index) => { //使用箭頭函數表示
            let cityData = orgData[city];
            region.innerHTML +=
                `
                <div class="flip-card">
                        <div class="flip-card-inner">
                            <div class="flip-card-front">
                                <h3>${city}</h3>
                                <p>${cityData.wx},${cityData.ci}</p>
                                <p>最低溫：${cityData.minT} 最高溫：${cityData.maxT}</p>
                                <p>降雨機率：${cityData.pop}%</p>
                                <p>${cityData.startTime.substr(0,16).replace('-', '/')} -${cityData.endTime.substr(0,16).replace('-', '/')}</p>
                            </div>
                            <div class="flip-card-back">
                                <p>${cityData.wx2},${cityData.ci2}</p>
                                <p>最低溫：${cityData.minT2} 最高溫：${cityData.maxT2}</p>
                                <p>降雨機率：${cityData.pop2}%</p>
    ...
    btn_middle.addEventListener('click', function () {
        region.innerHTML = '';
        cities[2].forEach((city, index) => { //使用箭頭函數表示
            let cityData = orgData[city];
            region.innerHTML +=
                `
                <div class="flip-card">
    ...
    btn_south.addEventListener('click', function () {
        region.innerHTML = '';
        cities[3].forEach((city, index) => { //使用箭頭函數表示
            let cityData = orgData[city];
            region.innerHTML +=
                `
                <div class="flip-card">
    ...
    btn_east.addEventListener('click', function () {
        region.innerHTML = '';
        cities[4].forEach((city, index) => { //使用箭頭函數表示
            let cityData = orgData[city];
            region.innerHTML +=
                `
                <div class="flip-card">
    ...
    btn_island.addEventListener('click', function () {
        region.innerHTML = '';
        cities[5].forEach((city, index) => { //使用箭頭函數表示
            let cityData = orgData[city];
            region.innerHTML +=
                `
            <div class="flip-card">
